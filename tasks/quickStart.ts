...
task('quickStart')
    .setAction(async (_, hre) => {
        ...
        // Choose network
        // Should work similar to the choose key procedure
        let validNetwork = false;
        let network: TestnetNetwork | undefined = undefined;
        while (!validNetwork) {
            let chosenNetwork = await rl.question("Which testnet network would you like to use? (dev / blue): ");
            chosenNetwork = chosenNetwork.trim().toLowerCase();
            switch (chosenNetwork) {
                case TestnetNetwork.BLUE:
                    network = TestnetNetwork.BLUE;
                    configFile = TESTNET_BLUE_CONFIG_FILE;
                    validNetwork = true;
                    break;
                case TestnetNetwork.DEV:
                    network = TestnetNetwork.DEV;
                    configFile = TESTNET_DEV_CONFIG_FILE;
                    validNetwork = true;
                    break;
                default:
                    validNetwork = false;
                    timestampLog("You have chosen an invalid network. Options: \"dev\" | \"blue\" ");
            }
        }
        ...
        timestampLog(`Balance for wallet ${walletAddress}: ${formattedBalance}`);
        rl.close();
